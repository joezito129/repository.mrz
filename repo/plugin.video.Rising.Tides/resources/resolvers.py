...
def resolve(url):
        if 'tvcatchup' in url:
            open_ = OPEN_URL(url)
            url  = re.compile("file: '(.+?)'").findall(open_)[0]
            ...
            open_  = OPEN_URL(url)
            iframe= regex_from_to(open_, "iframe src='", "'")
            h = {'referer': url}
            ...
            headers = {
                'Authorization': 'Basic QFN3aWZ0MTQjOkBTd2lmdDE0Iw==',
                'User-Agent': 'Dalvik/1.6.0 (Linux; U; Android 4.4.4; SM-G900F Build/KTU84Q)'
            }

            try:
                open_ = requests.session().get('http://173.212.202.101/token4004.php', headers=headers).text
            except:
                open_ = requests.session().get('http://173.212.202.101/token10304.php', headers=headers).text
            if '404 Not Found' in open_:
                open_ = requests.session().get('http://173.212.202.101/token7004.php', headers=headers).text
...
def swiftstreamsresolve(url):
    headers = {
        'Authorization': 'Basic U25hcHB5OkBTbmFwcHlA',
        'User-Agent': 'Dalvik/1.6.0 (Linux; U; Android 4.4.4; SM-G900F Build/KTU84Q)'
    }
