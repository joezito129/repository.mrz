...
class Torbox:
    ...
    def auth(self) -> None:
        ...
        r = requests.get(f'{self.BaseUrl}/user/auth/device/start', params=params, timeout=20)
        if r.ok:
    ...
    def auth_loop(self, data, timeout) -> tuple:
        ...
        r = requests.post(f'{self.BaseUrl}/user/auth/device/token', json=data, timeout=20)
        if r.ok:
    ...
    def status(self) -> bool:
        r = requests.get(f'{self.BaseUrl}/user/me', headers=self.headers(), timeout=20)
        if r.ok:
    ...
    def refreshToken(self):
        ...
        r = requests.post(url, headers=self.headers(), timeout=20)
        return r.ok
    ...
    def addMagnet(self, magnet: str) -> dict:
        ...
        r = requests.post(url, headers=self.headers(), data=data, timeout=20)
        return r.json()['data']
    ...
    def delete_torrent(self, torrent_id) -> bool:
        ...
        r = requests.post(url, headers=self.headers(), json=data, timeout=20)
        return r.ok
