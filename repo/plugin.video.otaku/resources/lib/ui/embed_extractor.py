...
def load_video_from_url(in_url):
    ...
    try:
        ...
        # control.print_(f'load_video_from_url: \n{reqObj}')
        return found_extractor['parser'](reqObj[5], reqObj[0], reqObj[2].get('Referer'))
...
def __check_video_list(refer_url, vidlist, add_referer=False, ignore_cookie=False):
    nlist = []
    headers = {}
    if add_referer:
    ...
    for item in vidlist:
        try:
            ...
            control.print_(f'check_video_list: \n{temp_req}')
            if temp_req[1] != '200':
...
def __extract_streamlare(url, page_content, referer=None):
    ...
    if source:
        ...
        if '?token=' in source:
            ...
            # r = requests.get(source, headers=headers, redirect=False)
            control.print_(f'streamlare3: \n{t}')
            if t:
...
def __extract_goload(url, page_content, referer=None):
    ...
    if r:
        ...
        r = requests.get(eurl, headers={"X-Requested-With": 'XMLHttpRequest'})
        if r.ok:
