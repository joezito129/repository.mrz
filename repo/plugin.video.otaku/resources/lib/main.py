...
def ANIMES_PAGE(payload, params):
    payload_list = payload.rsplit("/")
    if len(payload_list) == 2: anilist_id, mal_id = payload_list
    else:
...
def SEARCH(payload, params):
    ...
    if "Yes" in control.getSetting('searchhistory'): database.addSearchHistory(query, 'show')
...
def TOOLS_MENU(payload, params):
    ...
    return control.draw_items(
        [utils.allocate_item(name, url, True, image) for name, url, image in TOOLS_ITEMS], "addons")
...
def Main():
    ...
    if not player.player().isPlaying(): player.playList.clear()
