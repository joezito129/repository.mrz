...
class sources(BrowserBase):
    def get_sources(self, anilist_id, episode):
        ...
        all_results = []
        for lang in langs:
            r = database.get_(anify.ANIFYAPI().get_sources_json, 8, anilist_id, episode, 'animepahe', lang)
            if r and r.get('sources'):
                srcs = r['sources']
                subs = r.get('subtitles')
                if subs:
                    subs = [x for x in subs if x.get('lang') != 'Thumbnails']
                mapfunc = partial(anify.process_anify, provider='animepaheHD', title=title, lang=2 if lang == 'dub' else 0, subs=subs)
